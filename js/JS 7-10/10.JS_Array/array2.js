...
/*
    ...
    - 배열을 수노히하면서 각 요소와 인덱스를 사요할 수 있도록 함
    - 원본 배열 X
    - brake,continue, ... 부가적인 작업을 추가 X    
...
/*
    ...
    - 원본 배열 변경 X
    - forEach 단순히 요소를 가지고 작업만 진행 (반환갑 X), map 요소를 가지고 작업한 결과로 배열 새로 만듦 (반환값 O)
*/
...
*/
const findResult = array.find((item, index) => {
  return item % 2 === 1;
});
...
*/
const findIndexResult = array.findIndex((item, index) => {
  return item % 2 === 1;
});
